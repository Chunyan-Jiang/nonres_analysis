# -------------------------------------------------------------------
# Purpose:
#   Random Forest (RF) imputation for BS + simulation-based evaluation.
...
# Generate lagged versions of BS and BE (1–4 lags) within each firm (idnum).
...
discrete.covariates <- c('idnum','vg_westeast','sector_total','online','is_dec','is_aug','vg_statebus','vg_comexp'
                         ,'vg_statebus_lag1'
                           ...
                           ,'vg_comexp_lag3'
                           ,'vg_comexp_lag4')
continuous.covariates <- c('calendar_time')
filled.data <- filled.data%>%
...
for (k in BLOCK_LENGTHS) {
  ...
  save(list = ls(pattern = "^sim_result_w_online_"), file = 'outputs/RF results/RF_BS_res_temp.RData')
}
...
save(list = objects_to_save, file = 'outputs/RF results/RF_BS_res.RData')
...
rf_bs_balance_w_online <- ggplot(df_long, aes(x = shift_date, y = Balance, color = Type)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(
...
if(!dir.exists("outputs/RF results")) dir.create("outputs/RF results", recursive = TRUE)
ggsave("outputs/RF results/rf_bs_balance.pdf", plot = rf_bs_balance_w_online, width = 10, height = 6, units = 'in')
