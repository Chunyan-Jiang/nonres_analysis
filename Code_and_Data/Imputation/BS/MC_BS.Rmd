# -------------------------------------------------------------------
# Purpose:
#   Homogeneous Markov Chain (MC) imputation for BS + simulation-based evaluation.
#
# Workflow:
...
#   3) Run simulations for block_length = 1..6 (n_sim = 30, fixed seed) for different Markov orders (MC2–MC4; optional)
...
# Generate lagged versions of BS and BE within each firm (idnum).
# These lags are used to form lag-pattern conditions for the Markov Chain imputation.
# We convert vg_statebus and vg_comexp to character to treat them as discrete states consistently when constructing transition/conditional frequency tables.
```{r}
filled.data$vg_statebus <- as.character(filled.data$vg_statebus)
filled.data$vg_comexp <- as.character(filled.data$vg_comexp)
...
for (k in BLOCK_LENGTHS) {
  ...
  save(list = ls(pattern = "^sim_mc1_result_"), file = 'outputs/MC results/MC_BS_res1_temp.RData')
}
```
## MC2 (optional)
```{r}
for (k in BLOCK_LENGTHS) {
  message(paste("\n>>> Starting MC2 Scenario: Block Length =", k, "<<<"))
  ...
  # Intermediate save
  if(!dir.exists("outputs/MC results")) dir.create("outputs/MC results", recursive = TRUE)
  save(list = ls(pattern = "^sim_mc2_result_"), file = 'outputs/MC results/MC_BS_res2_temp.RData')
}
...
```{r}
for (k in BLOCK_LENGTHS) {
  message(paste("\n>>> Starting MC3 Scenario: Block Length =", k, "<<<"))
  ...
  # Intermediate save
  if(!dir.exists("outputs/MC results")) dir.create("outputs/MC results", recursive = TRUE)
  save(list = ls(pattern = "^sim_mc3_result_"), file = 'outputs/MC results/MC_BS_res3_temp.RData')
}
...
for (k in BLOCK_LENGTHS) {
  message(paste("\n>>> Starting MC4 Scenario: Block Length =", k, "<<<"))
  ...
  # Intermediate save
  if(!dir.exists("outputs/MC results")) dir.create("outputs/MC results", recursive = TRUE)
  save(list = ls(pattern = "^sim_mc4_result_"), file = 'outputs/MC results/MC_BS_res4_temp.RData')
}
```
...
save(list = objects_to_save, file = 'outputs/MC results/MC_BS_res.RData')
```
