# -------------------------------------------------------------------
# Purpose:
#   LOCF imputation for BE + simulation-based evaluation.
#
# Workflow:
...
for (k in BLOCK_LENGTHS) {
  ...
  current_result <- simulate_imputation_evaluation(
    ...
    impute_args = list(masked_var = "vg_comexp_masked", 
                       mask_flag = "is_masked", 
                       method = 'locf'),
    n_sim = N_SIM,
    seed = SEED
...
save(list = objects_to_save, file = 'outputs/LOCF results/LOCF_BE_res.Rdata')
```
...
df <- filled.data%>%select(idnum,shift_date,vg_comexp)

df.imputed <- df%>%
  arrange(idnum, shift_date) %>%
  group_by(idnum) %>%
  mutate(vg_comexp_imputed = na.locf(vg_comexp, na.rm = FALSE)) %>%
  ungroup()
...
locf_be_balance <- ggplot(df_long, aes(x = shift_date, y = Balance, color = Type)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(
...
if(!dir.exists("outputs/LOCF results")) dir.create("outputs/LOCF results", recursive = TRUE)
ggsave("outputs/LOCF results/locf_be_balance.pdf", plot = locf_be_balance, width = 10, height = 6, units = 'in')
