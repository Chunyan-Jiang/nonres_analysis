# -------------------------------------------------------------------
# Purpose:
#   Summarize and compare BE imputation performance across methods (LOCF, MC1, RF).
#
# Workflow:
#   1) Read simulation outputs (all_runs CSV) produced by the BE imputation scripts for block_length k = 1..6.
...
#
# Outputs:
#   - outputs/Metrics Comparision Results/be_kappa_w_online.pdf
#   - outputs/Metrics Comparision Results/be_accuracy_w_online.pdf
#   - outputs/Metrics Comparision Results/be_rho_w_online.pdf
# -------------------------------------------------------------------
...
read_and_process_locf <- function(k_value) {
  ...
  read_csv(file_path, show_col_types = FALSE) %>%
    ...
    mutate(question = "be", method = "LOCF", k = k_value) %>%
    select(question, method, k, kappa, accuracy, rho)
}

k_values <- 1:6
be_locf_combined <- map_dfr(k_values, read_and_process_locf)
```
...
read_and_process_mc <- function(k_value) {
  ...
  read_csv(file_path, show_col_types = FALSE) %>%
    rename(kappa = mean_kappa, accuracy = mean_accuracy, rho = mean_rho) %>%
    mutate(question = "be", method = "MC (1)", k = k_value) %>%
    select(question, method, k, kappa, accuracy, rho)
}

k_range <- 1:6

be_mc_combined <- map_dfr(k_range, read_and_process_mc)
...
read_and_process_rf <- function(k_value) {
  ...
  read_csv(file_path, show_col_types = FALSE) %>%
    rename(kappa = mean_kappa, accuracy = mean_accuracy, rho = mean_rho) %>%
    mutate(question = "be", method = "RF", k = k_value) %>%
    select(question, method, k, kappa, accuracy, rho)
}

be_rf_combined <- map_dfr(1:6, read_and_process_rf)
```

# Combine All Results
```{r}
be_combined <- bind_rows(be_locf_combined, be_mc_combined, be_rf_combined)
```
...
be_combined$method <- factor(be_combined$method, levels = method_order)


# Figure 1: Kappa - unified x-axis
p1 <- ggplot(be_combined, aes(x = kappa, y = factor(k), fill = method)) +
...
ggsave("outputs/Metrics Comparision Results/be_kappa_w_online.pdf", plot = p1,width = 10, height = 6, units = "in")

# Figure 2: Accuracy - unified x-axis
p2 <- ggplot(be_combined, aes(x = accuracy, y = factor(k), fill = method)) +
  geom_boxplot(outlier.size = 0.8, alpha = 0.8) +
  labs(
    x = "Accuracy",
    y = "Length of consecutive missing months",
...
if(!dir.exists("outputs/Metrics Comparision Results")) dir.create("outputs/Metrics Comparision Results", recursive = TRUE)
ggsave("outputs/Metrics Comparision Results/be_accuracy_w_online.pdf", plot = p2,width = 10, height = 6, units = "in")

# Figure 3: Rho - unified x-axis
p3 <- ggplot(be_combined, aes(x = rho, y = factor(k), fill = method)) +
  geom_boxplot(outlier.size = 0.8, alpha = 0.8) +
  labs(
    x = "Spearman’s rho",
    y = "Length of consecutive missing months",
...
if(!dir.exists("outputs/Metrics Comparision Results")) dir.create("outputs/Metrics Comparision Results", recursive = TRUE)
ggsave("outputs/Metrics Comparision Results/be_rho_w_online.pdf", plot = p3,width = 10, height = 6, units = "in")
# Display plots
print(p1)
print(p2)
print(p3)
