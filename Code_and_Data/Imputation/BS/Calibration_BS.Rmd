...
```{r}
load("outputs/RF results/RF_BS_res.RData")
```
    ...
    mutate(
      prob_bin = cut(pred_prob,
       ...
       class = dplyr::case_when(
        ...
        class = factor(class, levels = c('Good', 'Satisfactory', 'Bad'))
    ) %>%
...
for (k in 1:6) {
  ...
  p <- plot_calibration_rf(sim_res, 'vg_statebus')
  assign(output_var_name, p)
  ggsave(output_filename, plot = p, width = 10, height = 6, units = 'in')
  message("Saved BS Calibration Graph: ", output_filename)
}
```
